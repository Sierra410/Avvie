...
notify = Notify.Notification.new(
    app_title, "Image file exported to Downloads."
)
notify_invalid_output = Notify.Notification.new(
    app_title, "Could not locate output folder!"
)
...
notify.add_action(
    "action_click", "Open output folder", open_encode_out, None
)
...
class FileChooserWithImagePreview(Gtk.FileChooserNative):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.connect(
            "update-preview", self.update_preview, self.preview_widget
        )
...
class Picture:
    def __init__(self):
        ...
        self.download_folder = GLib.get_user_special_dir(
            GLib.UserDirectory.DIRECTORY_DOWNLOAD
        )
        self.pictures_folder = GLib.get_user_special_dir(
    ...
    def test_br(self, x, y):
        ...
        return point_in_rect(
            picture.display_x + tx, picture.display_y + ty, tw, th, x, y
        )
    ...
    def test_center_start_drag(self, x, y):
        ...
        return point_in_rect(
            picture.display_x + rx, picture.display_y + ry, rw, rh, x, y
        )
        ...
        if self.sharpen:
            im = im.filter(
                ImageFilter.UnsharpMask(
                    radius=0.35, percent=150, threshold=0
                )
            )
        ...
        if self.crop:
            cr = im.crop(
                (
                    self.rec_x,
                    self.rec_y,
                    self.rec_x + self.rec_w,
                    self.rec_y + self.rec_h,
                )
            )
        else:
            cr = im.copy()
    ...
    def reload(self, keep_rect=False):
        ...
        if self.rotation:
            im = im.rotate(
                self.rotation, expand=True, resample=Image.NEAREST
            )  # , resample=0)
    ...
    def set_ratio(self):
        ...
        if self.crop_ratio and self.crop_ratio != (1, 1):
            ...
            if (
                self.crop_ratio == (21, 9)
                and abs(self.rec_h - 1080) < 50
            ):
                self.rec_h = 1080
                self.rec_w = 2560
    ...
    def get_display_rect(self):
        ...
        return (
            round(self.rec_x * self.scale_factor),
    ...
    def export(self, path=None):
        ...
        if self.export_constrain:
            cr.thumbnail(
                (self.export_constrain, self.export_constrain),
                Image.ANTIALIAS,
            )
...
class SettingsDialog(Gtk.Dialog):
    ...
    def __init__(self, parent):
        ...
        # ====================
        opt = Gtk.RadioButton.new_with_label_from_widget(
            None, "Export to Downloads"
        )
        opt.connect(
            "toggled", self.toggle_menu_setting_export, "download"
        )
        ...
        # ====================
        opt = Gtk.RadioButton.new_with_label_from_widget(
            opt, "Export to Pictures"
        )
        opt.connect(
            "toggled", self.toggle_menu_setting_export, "pictures"
        )
        vbox.pack_start(child=opt, expand=True, fill=False, padding=4)
        ...
        # ====================
        opt = Gtk.RadioButton.new_with_label_from_widget(
            opt, "Overwrite Source File"
        )
        opt.connect(
            "toggled", self.toggle_menu_setting_export, "overwrite"
        )
        vbox.pack_start(child=opt, expand=True, fill=False, padding=4)
        ...
        # ====================
        vbox.pack_start(
            child=Gtk.Separator(), expand=True, fill=False, padding=4
        )
        ...
        inline_box.pack_start(
            child=b, expand=True, fill=False, padding=0
        )
        ...
        parent.add_preview_adjustment = Gtk.Adjustment(
            value=64, lower=16, upper=512, step_increment=16
        )
        spinbutton.set_adjustment(parent.add_preview_adjustment)

        inline_box.pack_start(
            child=spinbutton, expand=True, fill=False, padding=4
        )

        # ####################
        vbox.pack_start(
            child=inline_box, expand=True, fill=False, padding=2
        )
...
class Window(Gtk.Window):
    def __init__(self):
        ...
        self.rot = Gtk.Scale.new_with_range(
            orientation=0, min=-90, max=90, step=2
        )
    ...
    def set_export_text(self):
        ...
        if setting == "download":
            self.quick_export_button.set_tooltip_text(
    ...
    def setup_window(self):
        ...
        self.custom_resize_radio = (
            Gtk.RadioButton.new_with_label_from_widget(opt, "Custom")
        )
        self.custom_resize_radio.connect(
            "toggled", self.toggle_menu_setting, "custom"
        )
        inline_box.pack_start(
            child=self.custom_resize_radio,
            expand=True,
            fill=False,
            padding=0,
        )

        self.custom_resize_adjustment = Gtk.Adjustment(
            value=1920, lower=2, upper=10000, step_increment=50
        )
        self.custom_resize_adjustment.connect(
            "value-changed", self.set_custom_resize
        )
        ...
        vbox.pack_start(
            child=inline_box, expand=True, fill=False, padding=0
        )
        ...
        vbox.pack_start(
            child=self.rot, expand=True, fill=False, padding=7
        )
        vbox.pack_start(
            child=self.rotate_reset_button,
            expand=True,
            fill=False,
            padding=7,
        )
        ...
        vbox.pack_start(
            child=flip_vert_button, expand=True, fill=False, padding=2
        )
        flip_hoz_button = Gtk.Button(label="Flip Horizontal")
        flip_hoz_button.connect("clicked", self.toggle_flip_hoz)
        vbox.pack_start(
            child=flip_hoz_button, expand=True, fill=False, padding=2
        )
        ...
        self.about.set_copyright(
            "Copyright 2019 Taiko2k captain.gxj@gmail.com"
        )
        self.about.set_license_type(Gtk.License(3))
        self.about.set_website(
            "https://github.com/taiko2k/" + app_title.lower()
        )
        ...
        for item in sys.argv:
            if not item.endswith(".py") and os.path.isfile(item):
                ...
                self.discard_exif_button.set_sensitive(
                    picture.exif and True
                )
                break
    ...
    def on_key_press_event(self, widget, event):
        ...
        if (
            event.keyval == Gdk.KEY_Shift_L
            or event.keyval == Gdk.KEY_Shift_R
        ):
        ...
        if (
            ...
            and not self.free_rectangle_radio.get_active()
        ):
    ...
    def toggle_menu_setting(self, button, name):
        ...
        if name == "custom" and button.get_active():
            picture.export_constrain = int(
                self.custom_resize_adjustment.get_value()
            )
    ...
    def click(self, draw, event):
        ...
        for i, size in enumerate(picture.thumbs):
            ...
            if (
                right - size < event.x < right
                and bottom - size < event.y < bottom
            ):
                if event.button == 1:
                    picture.circle ^= True
                ...
                if event.button == 3:
                    ...
                    self.thumb_menu_remove.set_label(
                        f"Remove {size}x{size}"
                    )
                    if picture.circle:
                        self.circle_menu_item.set_label(
                            "Square Preview"
                        )
                    else:
                        self.circle_menu_item.set_label(
        ...
        if event.button == 1:
            ...
            if (
                picture.get_display_rect_hw() < picture.all_drag_min
                and picture.test_center_start_drag(event.x, event.y)
            ):
    ...
    def mouse_motion(self, draw, event):
        ...
        if event.state & Gdk.ModifierType.BUTTON1_MASK and picture.crop:
            ...
            dragging_corners = bool(
                picture.dragging_tl
                or picture.dragging_bl
                or picture.dragging_br
                or picture.dragging_tr
            )
    ...
    def draw(self, wid, c):
        ...
        if picture.ready:
            ...
            if picture.crop:
                ...
                if picture.rec_h == 1080 and (
                    picture.rec_w == 2560 or picture.rec_w == 1920
                ):
                    c.set_source_rgba(0.2, 0.9, 0.2, 1)
                elif picture.lock_ratio and picture.crop_ratio != (
                    1,
                    1,
                ):
                    if (
                        picture.rec_w
                        / picture.crop_ratio[0]
                        * picture.crop_ratio[1]
                        == picture.rec_h
                    ):
            ...
            if picture.thumb_surfaces:
                ...
                for i, size in enumerate(picture.thumbs):
                    ...
                    if (
                        i == 0
                        and picture.exif
                        and not picture.discard_exif
                        and picture.png is False
                    ):
